...
const ShopNearMe = ({ navigation }) => {
    ...
    const Item = ({ item }) => {
        ...
        let color = item.item.status === "Accepting Orders" ? 'green' : 'red'

        return <View style={{ width: '100%', height: 150, marginTop: 10, marginVertical: 60 }}>
            <Pressable onPress={() => { navigation.navigate('drink') }}>
                ...
                <View style={{
                    ...
                    <View style={{
                        ...
                        <Ant name={item.item.status === "Accepting Orders" ? 'check' : 'lock'}
                            style={{
                                color: color,
                                fontSize: 26
                            }} />
                    ...
                    <View style={{
                        ...
                        <Text>{item.item.timeToOrder}</Text>
                    </View>
                    <EvilIcons
                        name="location"
                        style={{
                ...
                <Text style={{ fontSize: 20, fontWeight: '600' }}>{item.item.name}</Text>
                <Text style={{}}>{item.item.address}</Text>
            </Pressable>
        </View>
    }
